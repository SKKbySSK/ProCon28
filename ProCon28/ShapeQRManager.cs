...
namespace ProCon28
{
    ...
    static class LocationQRManager
    {
        static List<string> shapes = new List<string>();
        public static string[] GetShapes()
        {
        ...
        public static Linker.CombinedBlackPiece GenerateCompositePiece()
        {
            Linker.PieceCollection pcol = new Linker.PieceCollection();
            foreach (string qr in shapes)
                pcol.AddRange(GeneratePieces(qr));

            return new Linker.CombinedBlackPiece(pcol);
        }
    }
}
